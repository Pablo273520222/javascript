...
export const getLastPrincipalOfDirector = () => {
  const LASTFILM = movie.director.notableFilmography.length;
...
export const getDescriptionOfGandalf = () => {
  const gandalf = movie.characters.find((character) => character.name === "Gandalf");
  return gandalf ? gandalf.description : "Character not found.";
};
 ...
 */
export const getRealNameOfAragorn = () => {
  const aragorn = movie.characters.find((character) => character.name === "Aragorn");
  return aragorn ? aragorn.actor : "Character not found.";
};
 ...
 */
export const getIndexOfFantasyValue = () => {
  const index = Object.values(movie).indexOf("Fantasía");
  return index;
};
